...
static napi_value VibrateEffect(napi_env env, napi_value args[], size_t argc)
{
    ...
    if (!ParseParameter(env, args, argc, info)) {
        ThrowErr(env, PARAMETER_ERROR, "Parameter verification failed", "parameter fail");
        return nullptr;
    }
    sptr<AsyncCallbackInfo> asyncCallbackInfo = new (std::nothrow) AsyncCallbackInfo(env);
    CHKPP(asyncCallbackInfo);
    ...
    if ((asyncCallbackInfo->error.code != SUCCESS) && (asyncCallbackInfo->error.code == PARAMETER_ERROR)) {
        ThrowErr(env, PARAMETER_ERROR, "Parameter verification failed", "parameters invalid");
        return nullptr;
    }
...
static napi_value StopVibrationSync(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        ThrowErr(env, PARAMETER_ERROR, "Parameter verification failed", "Get the parameter info fail");
        return result;
    }
    int32_t ret = Cancel();
    if (ret != SUCCESS) {
...
static napi_value IsHdHapticSupported(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        ThrowErr(env, PARAMETER_ERROR, "Parameter verification failed", "Must get the value of boolean");
    }
    return result;
}
...
static napi_value IsSupportEffect(napi_env env, napi_callback_info info)
{
    ...
    if ((status != napi_ok) || (argc == 0)) {
        ThrowErr(env, PARAMETER_ERROR, "Parameter verification failed",
            "parameter must be greater than 2");
        return nullptr;
    }
    string effectId;
...
static napi_value IsSupportEffectSync(napi_env env, napi_callback_info info)
{
    ...
    string effectId;
    if (!GetStringValue(env, args[0], effectId)) {
        ThrowErr(env, PARAMETER_ERROR, "Parameter verification failed", "Must get the value of string");
        return result;
    }
